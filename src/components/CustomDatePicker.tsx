...
>(({ label, error, className = '', value, onChange, ...props }, ref) => {
  ...
  const [calendarHeight] = useState(320);
  ...
  useLayoutEffect(() => {
    if (isOpen && containerRef.current) {
      ...
      if (spaceBelow < calendarHeight + 20 && rect.top > calendarHeight + 20) {
        setDirection('up');
      } else {
        setDirection('down');
      }
    }
  }, [isOpen, calendarHeight]);
  ...
  return (
    <div className={`relative w-full ${className}`} ref={containerRef}>
      ...
      <div className="relative group">
        ...
        >
          <CalendarIcon className={`w-4 h-4 ${iconColorClass}`} />
        </div>
      </div>
      ...
      <div
        className={`
          absolute left-0 z-50 w-64 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-md shadow-xl overflow-hidden ease-out origin-top
