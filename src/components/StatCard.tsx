    ...
    return (
        <Link to={to} className="flex items-center bg-white dark:bg-dark-card rounded-lg shadow-md overflow-hidden transition-transform duration-200 hover:scale-105">
            <div className={`p-7 ${isDanger ? 'bg-red-500' : 'bg-lumi-primary'}`}>
                <img src={iconUrl} alt={title} className="w-10 h-10 select-none" />
            </div>
            <div className="p-4 flex flex-col justify-center">
                <p className={`text-base font-semibold select-none ${isDanger ? 'text-red-500' : 'text-gray-600 dark:text-gray-300'}`}>{title}</p>
                <p className={`text-4xl font-bold select-none ${isDanger ? 'text-red-600' : 'text-gray-800 dark:text-white'}`}>{value}</p>
            </div>
        </Link>
    );
}