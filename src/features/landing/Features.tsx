  ...
  return (
    ...
    <div className="mt-5 flex flex-wrap items-center gap-2">
      {kinds.map((kind) => (
        <span
          key={kind.label}
          className={`inline-flex items-center rounded-control ${kind.tone} px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.06em] text-white`}
        >
          {kind.label}
        </span>
      ))}
      <span className="inline-flex items-center rounded-control border border-dashed border-paper-400 px-2.5 py-1 font-mono text-[11px] text-paper-600 dark:border-white/20 dark:text-ink-400">
      ...
      <span className="inline-flex items-center gap-1.5 rounded-control bg-amber-100 px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.06em] text-amber-800 dark:bg-amber-400/15 dark:text-amber-300">
  ...
  return (
    <ul className="mt-5 flex flex-wrap gap-1.5">
      {REPORT_KEYS.map((key) => (
        <li
          key={key}
          className="rounded-control border border-paper-300 px-2.5 py-1 font-mono text-[11px] text-paper-600 dark:border-white/10 dark:text-ink-400"
        >
            ...
            >
              ...
              >
                <div className={item.wide ? 'md:w-1/3' : ''}>
                  <span className="mb-4 flex h-8 w-8 items-center justify-center rounded-control border border-lumi-500/35 bg-lumi-50 text-lumi-600 dark:border-lumi-label/30 dark:bg-lumi-500/15 dark:text-lumi-200">
